...
function getTable(users){
	...
	socket.emit('sendTopPlayers');
	socket.on('topPlayers', function (data) {
		console.log(data.ENEMY_LIST);
		users = data.ENEMY_LIST;
	    setTimeout(function() {getTable(users)},3000);
	...
	Crafty.defineScene("Scene1", function () {
		...
		var door = Crafty.e('2D, DOM, Color, Door').attr({
				x: 3326,
				y: 256 + 83,
				w: 80,
				h: 100
